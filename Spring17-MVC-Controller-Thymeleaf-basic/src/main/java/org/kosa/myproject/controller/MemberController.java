...
public class MemberController {
  ...
  public String findMemberById(@RequestParam String memberId, Model model) {
    //src/main/resources/templates/member/findbyid-result.html로 Thymeleaf ViewResolver가 찾아서 파싱하고 렌더링한다.    
    ...
    // client에서 보낸 텍스트 입력폼의 name과 매개변수명이 일치하면 request.getParameter("memberId"); 처리를 해준다.
    return "member/findbyid-result"; // view name
  }
  @GetMapping("/find-by-id2")
  public String findMemberById2(String memId,Model model) {
...
//  // url 매핑
//  public String paramTest(@RequestParam String nick,@RequestParam int age,@RequestParam Model model) { // int로 자동 형변환 : HandlerAdapter가 변환해 전달
//  // 동작할 메서드 paramTest 작성
//    if(age>18) {
...
//  }
  ...
  public String register(Member member) { // form에서 전송한 회원 정보가 객체로 생성되어 매개변수로 전달된다. : HandlerAdapter
    System.out.println("회원 가입 정보 등록:"+member);
    //return "member/register-result"; // forward 방식, 부적합 request가 유지된다. 기존 데이터가 새로 고침시 다시 입력(재동작)
