...
public class MyTestController {
  ...
  @GetMapping("/paramTest") // HandlerMapping url 등록해서 찾아준다.
  public String paramTest(String customerId, int customerAge, Model model) { // HandlerAdapter가 호출
    // get으로 들어오는 데이터 매개변수로 받으면 된다.
    // request 객체로 데이터를 받기 위해서 model 객체 사용
   ...
   * 이 것을 HandlerAdapter가 요청 데이터에 대한 배열 객체를 생성해서 우리의 컨트롤러
   * 메서드에 전달해 준다.
   */
  @PostMapping("/paramTest2")
  public String paramTest2(String[] menu) {
    System.out.println("매뉴 수:"+menu.length);
    // redirect 방식은 클라이언트에게 url을 지정해서 응답받게 한다.
    return "redirect:/result3-view";
    // Spring 환경에서 redirect는 FrontController인
  ...
  public String hasATest(Customer customer, Model model) { // HadlerAdapter가 만들어서 준다.
  ...
  public String sessionTest1(HttpSession session) {// HandlerAdapter가 만들어서 준다
