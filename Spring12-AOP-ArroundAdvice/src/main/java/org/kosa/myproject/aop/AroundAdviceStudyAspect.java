...
public class AroundAdviceStudyAspect {
  ...
  public Object testAround(ProceedingJoinPoint point) throws Throwable {
    // 실제 Core를 수행, 만약 Arround Advice 환경에서 아래 코드를 수행하지 않으면
    // 반드시 명시해야 한다. 아래 코드를 명시하지 않으면 Core가 실행되지 않는다.ㅉ

    logger.debug("before advice 메서드 실행전 횡단관심사 로직 적용");
    ...
    if(retValue!=null) {
      ...
      //retValue+="불목~";
      logger.debug("AOP After-Returning** return value: {}",retValue); // 정상적으로 리턴할때 실행하는 advise
    }
    } catch(InventoryException ie) { // after throwing : 재고부족으로 예외 발생할때 대체
      // 공급처에 정보 전달
      logger.error("**AOP After-Throwing**message:{}",ie.getMessage(),ie);
      throw ie;
    } finally {
      logger.error("**AOP After**"); // 예외 발생 여부와 관계없이 항상
    }
    return retValue;
  }
}