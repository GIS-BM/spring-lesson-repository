...
//@Component
//@Order(2) // Runner가 여러개 있을 때 우선순위를 줄 수 있다.
// 기본 MyBatis 예저는 마무리 되었으므로 실행에서 일단 제외 시킨다.
...
//    logger.debug("총 상품수:{}", totalProductCount);
    /*
     * 현 러너에서 DI : ProductMapper org.kosa.myproduct.mapper.ProductMapper interface를
     * 정의한다. src/main/resources 아래에 동일한 패키지에 ProductMapper xml 정의한다.
     */
      // 전체 상품 조회
...
//      logger.debug("수정된 상품정보 확인 {}", product2);
    ...
    // 상품 등록 후 primary key인 product_id에 자동 생성된 정보를 확보했는지 확인
...
//    logger.debug("총 상품수 {}", productMapper.getTotalProductCount()); // 11
