...
public class Spring10LoggingApplication implements CommandLineRunner {
  ...
  private void testLogging() {
    ...
    logger.debug("debug 디버깅을 위한 정보");// 디버그 레벨로 설정
  ...
  private void simulateLoginService(String id) {
    /*
     * {} 를 이용한 파라미터 바인딩 -> 성능상 훨씬 유리 ( String을 + 거 보다 )
     * 내부적으로 StringBuilder를 이용,
     * 또한 실행 로깅 레벨이 아니면 문자열 결합하지 않는다.
     */
    logger.info("사용자 로그인 시도:{} 메세지 {}", id,"logging study");
    if(id.equals("java")) {
      // db에서 조회해옴
      String role = "CafeManager";
      logger.info("사용자 {} (역할:{}) 로그인 성공",id, role);
    } else {
    ...
    riskOperation();
      ...
      //e.printStackTrace(); // 정보가 지속되지 않는다.
